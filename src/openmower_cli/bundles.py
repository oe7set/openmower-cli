...
def apply_env(bundle: Bundle) -> None:
    ...
    Tags are always written. Image keys are written only when the bundle pins a
    non-default image; otherwise they are unset so compose falls back to the
    `${VAR:-default}` value. We never write empty values, because `:-default`
    only triggers when a variable is unset or empty.
    """
    ...
    if bundle.ros.image and bundle.ros.image != DEFAULT_ROS_IMAGE:
        set_key(path, ROS_IMAGE_KEY, bundle.ros.image)
    else:
        unset_key(path, ROS_IMAGE_KEY)

    if bundle.app.image and bundle.app.image != DEFAULT_APP_IMAGE:
...
# Old hardcoded image lines shipped by OpenMowerOS, and their switchable forms.
_ROS_OLD_RE = re.compile(
    r'^(?P<indent>\s*)image:\s*"?ghcr\.io/clemenselflein/open_mower_ros:\$\{VERSION\}"?\s*$'
)
_APP_OLD_RE = re.compile(
    r'^(?P<indent>\s*)image:\s*"?ghcr\.io/clemenselflein/openmowerapp:latest"?\s*$'
)
_ROS_NEW = '{indent}image: "${{ROS_IMAGE:-ghcr.io/clemenselflein/open_mower_ros}}:${{VERSION:-latest}}"'
_APP_NEW = '{indent}image: "${{APP_IMAGE:-ghcr.io/clemenselflein/openmowerapp}}:${{APP_VERSION:-latest}}"'


def compose_is_switch_ready(text: str) -> bool:
    """True once both image lines reference the switchable ROS_IMAGE/APP_IMAGE vars."""
    return "${ROS_IMAGE" in text and "${APP_IMAGE" in text


def migrate_compose_text(text: str) -> tuple[str, bool]:
    """Rewrite the two hardcoded image lines to their switchable forms.
    ...
    for line in text.splitlines(keepends=True):
        ...
        m_ros = _ROS_OLD_RE.match(body)
        m_app = _APP_OLD_RE.match(body)
        if m_ros and "${ROS_IMAGE" not in body:
            out.append(_ROS_NEW.format(indent=m_ros.group("indent")) + nl)
            changed = True
        elif m_app and "${APP_IMAGE" not in body:
            out.append(_APP_NEW.format(indent=m_app.group("indent")) + nl)
            changed = True
        else:
            out.append(line)
